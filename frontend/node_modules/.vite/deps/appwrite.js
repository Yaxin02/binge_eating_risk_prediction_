...
//#endregion
//#region node_modules/appwrite/dist/esm/sdk.js
var import_json_bigint = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function reviver(_key, value) {
	if (isBigNumber(value)) {
		if (value.isInteger()) {
			...
			const bi = BigInt(str);
			if (bi >= MIN_SAFE && bi <= MAX_SAFE) return Number(str);
			if (bi >= MIN_INT64 && bi <= MAX_INT64) return bi;
...
var Client = class Client {
		...
		this.realtime = {
			...
			createHeartbeat: () => {
				...
				this.realtime.heartbeat = window === null || window === void 0 ? void 0 : window.setInterval(() => {
					var _a;
					(_a = this.realtime.socket) === null || _a === void 0 || _a.send(JSONbig.stringify({ type: "ping" }));
				}, 2e4);
			},
			createSocket: () => {
				var _a, _b, _c, _d;
				...
				let queryParams = "project=" + encodeURIComponent((_b = this.config.project) !== null && _b !== void 0 ? _b : "");
				...
				if (url !== this.realtime.url || !this.realtime.socket || ((_c = this.realtime.socket) === null || _c === void 0 ? void 0 : _c.readyState) > WebSocket.OPEN) {
				...
				try {
					...
					switch (message.type) {
						case "connected": {
							...
							if (session && !(messageData === null || messageData === void 0 ? void 0 : messageData.user)) (_b = this.realtime.socket) === null || _b === void 0 || _b.send(JSONbig.stringify({
								type: "authentication",
								data: { session }
							}));
							break;
						...
						case "event": {
							...
							if (!(data === null || data === void 0 ? void 0 : data.channels)) break;
							const eventSubIds = data.subscriptions;
							if (eventSubIds && eventSubIds.length > 0) for (const subscriptionId of eventSubIds) {
								const slot = this.realtime.subscriptionIdToSlot.get(subscriptionId);
								if (slot !== void 0) {
									const subscription = this.realtime.subscriptions.get(slot);
									if (subscription) setTimeout(() => subscription.callback(data));
								}
							}
							else {
								if (!data.channels.some((channel) => this.realtime.channels.has(channel))) break;
	...
	*/
	setEndpoint(endpoint) {
		if (!endpoint || typeof endpoint !== "string") throw new AppwriteException("Endpoint must be a valid string");
	...
	*/
	setEndpointRealtime(endpointRealtime) {
		if (!endpointRealtime || typeof endpointRealtime !== "string") throw new AppwriteException("Endpoint must be a valid string");
	...
	*/
	subscribe(channels, callback, queries = []) {
		const channelStrings = (Array.isArray(channels) ? channels : [channels]).map((ch) => {
			if (typeof ch === "string") return ch;
			if (ch && typeof ch.toString === "function") return ch.toString();
			return String(ch);
		});
	...
	call(method_1, url_1) {
		return __awaiter(this, arguments, void 0, function* (method, url, headers = {}, params = {}, responseType = "json") {
			...
			if ((_a = response.headers.get("content-type")) === null || _a === void 0 ? void 0 : _a.includes("application/json")) data = JSONbig.parse(yield response.text());
			else if (responseType === "arrayBuffer") data = yield response.arrayBuffer();
			else data = { message: yield response.text() };
			if (400 <= response.status) {
				let responseText = "";
				if (((_b = response.headers.get("content-type")) === null || _b === void 0 ? void 0 : _b.includes("application/json")) || responseType === "arrayBuffer") responseText = JSONbig.stringify(data);
			...
			if (data && typeof data === "object") data.toString = () => JSONbig.stringify(data);
			return data;
		});
	}
...
var Account = class {
	...
	*/
	...
	createJWT(paramsOrFirst) {
		let params;
		if (!paramsOrFirst || paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst)) params = paramsOrFirst || {};
		else params = { duration: paramsOrFirst };
	...
	updateMfaAuthenticator(paramsOrFirst, ...rest) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst) && ("type" in paramsOrFirst || "otp" in paramsOrFirst)) params = paramsOrFirst || {};
		else params = {
			type: paramsOrFirst,
			otp: rest[0]
		};
	...
	*/
	...
	createOAuth2Session(paramsOrFirst, ...rest) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst) && ("provider" in paramsOrFirst || "success" in paramsOrFirst || "failure" in paramsOrFirst || "scopes" in paramsOrFirst)) params = paramsOrFirst || {};
		else params = {
...
var Avatars = class {
	...
	getBrowser(paramsOrFirst, ...rest) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst) && ("code" in paramsOrFirst || "width" in paramsOrFirst || "height" in paramsOrFirst || "quality" in paramsOrFirst)) params = paramsOrFirst || {};
		else params = {
			code: paramsOrFirst,
			width: rest[0],
			height: rest[1],
	...
	getScreenshot(paramsOrFirst, ...rest) {
		...
		if (typeof viewportWidth !== "undefined") payload["viewportWidth"] = viewportWidth;
		if (typeof viewportHeight !== "undefined") payload["viewportHeight"] = viewportHeight;
		if (typeof scale !== "undefined") payload["scale"] = scale;
		if (typeof theme !== "undefined") payload["theme"] = theme;
		if (typeof userAgent !== "undefined") payload["userAgent"] = userAgent;
		if (typeof fullpage !== "undefined") payload["fullpage"] = fullpage;
		if (typeof locale !== "undefined") payload["locale"] = locale;
		...
		if (typeof accuracy !== "undefined") payload["accuracy"] = accuracy;
		if (typeof touch !== "undefined") payload["touch"] = touch;
		if (typeof permissions !== "undefined") payload["permissions"] = permissions;
		if (typeof sleep !== "undefined") payload["sleep"] = sleep;
		if (typeof width !== "undefined") payload["width"] = width;
		if (typeof height !== "undefined") payload["height"] = height;
		if (typeof quality !== "undefined") payload["quality"] = quality;
...
var Functions = class {
	...
	createExecution(paramsOrFirst, ...rest) {
		...
		if (typeof headers !== "undefined") payload["headers"] = headers;
		if (typeof scheduledAt !== "undefined") payload["scheduledAt"] = scheduledAt;
		const uri = new URL(this.client.config.endpoint + apiPath);
		return this.client.call("post", uri, { "content-type": "application/json" }, payload);
	}
...
var Graphql = class {
	...
	query(paramsOrFirst) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst) && "query" in paramsOrFirst) params = paramsOrFirst || {};
		else params = { query: paramsOrFirst };
...
var Teams = class {
	...
	get(paramsOrFirst) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst)) params = paramsOrFirst || {};
		else params = { teamId: paramsOrFirst };
		const teamId = params.teamId;
		if (typeof teamId === "undefined") throw new AppwriteException("Missing required parameter: \"teamId\"");
...
var Realtime = class {
	...
	*/
	...
	createSocket() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			for (const subscription of this.activeSubscriptions.values()) for (const channel of subscription.channels) allChannels.add(channel);
			let queryParams = `project=${projectId}`;
			for (const channel of allChannels) queryParams += `&channels[]=${encodeURIComponent(channel)}`;
			const selectAllQuery = Query.select(["*"]).toString();
			for (const [slot, subscription] of this.activeSubscriptions) {
				...
				for (const channel of subscription.channels) for (const query of queries) queryParams += `&${encodeURIComponent(channel)}[${slot}][]=${encodeURIComponent(query)}`;
			}
			...
			if (this.socket) {
				this.reconnect = false;
				if (this.socket.readyState < WebSocket.CLOSING) yield this.closeSocket();
				this.reconnect = true;
			}
	...
	*/
	channelToString(channel) {
		...
		if (channel && typeof channel.toString === "function") return channel.toString();
	...
	subscribe(channelsOrChannel_1, callback_1) {
		return __awaiter(this, arguments, void 0, function* (channelsOrChannel, callback, queries = []) {
			const channelStrings = (Array.isArray(channelsOrChannel) ? channelsOrChannel : [channelsOrChannel]).map((ch) => this.channelToString(ch));
			const channels = new Set(channelStrings);
			const queryStrings = [];
			for (const q of queries !== null && queries !== void 0 ? queries : []) if (Array.isArray(q)) for (const inner of q) queryStrings.push(typeof inner === "string" ? inner : inner.toString());
			else queryStrings.push(typeof q === "string" ? q : q.toString());
			...
			return { close: () => __awaiter(this, void 0, void 0, function* () {
				...
				if (subscriptionId) this.subscriptionIdToSlot.delete(subscriptionId);
				yield this.createSocket();
			}) };
		});
	}
	...
	handleResponseEvent(message) {
		...
		if (!channels || !events || !payload || !subscriptions || subscriptions.length === 0) return;
...
function normalize(id) {
	if (id === void 0 || id === null) throw new Error("Channel ID is required");
	const trimmed = String(id).trim();
	if (trimmed === "") throw new Error("Channel ID is required");
	return trimmed;
}
var Channel = class Channel {
	...
	next(segment, id) {
		return new Channel(id === void 0 ? [...this.segments, segment] : [
			...this.segments,
			segment,
			normalize(id)
		]);
